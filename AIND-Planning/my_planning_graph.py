...
class PgNode_s(PgNode):
    ...
    def __init__(self, symbol: str, is_pos: bool):
        ...
        self.symbol = symbol
        self.is_pos = is_pos
        self.hash_val = hash(self.symbol) ^ hash(self.is_pos)
...
class PgNode_a(PgNode):
    ...
    def __init__(self, action: Action):
       ...
       '''
        PgNode.__init__(self)
        self.action = action
        self.hash_val = hash(self.action.name) ^ hash(self.action.args)
        self.prenodes, self.preconds_pos, self.preconds_neg = self.precond_s_nodes()
        self.effnodes, self.effects_pos, self.effects_neg = self.effect_s_nodes()
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #   for example, the A0 level will iterate through all possible actions for the problem and add
        #   a PgNode_a to a_levels[0]
        ...
        s_nodes = set(self.s_levels[level])
        for action in self.all_actions:
            a_node = PgNode_a(action)

            if a_node.prenodes <= s_nodes:
    ...
    def add_literal_level(self, level):
        ...
        #s_nodes = map(lambda x: s_nodes | x.effnodes, self.a_levels[level - 1])
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """a1_effects_pos = set(node_a1.action.effect_add)
        a2_effects_neg = set(node_a2.action.effect_rem)

        if not a1_effects_pos.isdisjoint(a2_effects_neg):
            return True

        a1_effects_neg = set(node_a1.action.effect_rem)
        a2_effects_pos = set(node_a2.action.effect_add)

        if not a2_effects_pos.isdisjoint(a1_effects_neg):
            return True

        return False"""

        if not node_a1.effects_pos.isdisjoint(node_a2.effects_neg):
            return True

        if not node_a2.effects_pos.isdisjoint(node_a1.effects_neg):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """a1_effects_pos = set(node_a1.action.effect_add)
        a2_preconds_neg = set(node_a2.action.precond_neg)

        if not a1_effects_pos.isdisjoint(a2_preconds_neg):
            return True

        a2_effects_pos = set(node_a2.action.effect_add)
        a1_preconds_neg = set(node_a1.action.precond_neg)

        if not a2_effects_pos.isdisjoint(a1_preconds_neg):
            return True

        a1_effects_neg = set(node_a1.action.effect_rem)
        a2_preconds_pos = set(node_a2.action.precond_pos)

        if not a1_effects_neg.isdisjoint(a2_preconds_pos):
            return True

        a1_preconds_pos = set(node_a1.action.precond_pos)
        a2_effects_neg = set(node_a2.action.effect_rem)

        if not a2_effects_neg.isdisjoint(a1_preconds_pos):
            return True"""

        if not node_a1.effects_pos.isdisjoint(node_a2.preconds_neg):
            return True

        if not node_a2.effects_pos.isdisjoint(node_a1.preconds_neg):
            return True

        if not node_a1.effects_neg.isdisjoint(node_a2.preconds_pos):
            return True

        if not node_a2.effects_neg.isdisjoint(node_a1.preconds_pos):
            return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for s1_node in node_a1.parents:
            for s2_node in node_a2.parents:
                if s1_node.is_mutex(s2_node):
                    return True
    ...
    def h_levelsum(self) -> int:
        ...
        #goals = [PgNode_s(goal, True) for goal in self.problem.goal]
        goals = []
        for goal in self.problem.goal:
            # Check is the goal is a negative literal
            if goal.op is '~':
                # if so, get the positive literal from the goal
                # and create a negative S node
                goals.append(PgNode_s(goal.args[0], False))
            else:
                # if not, create a positive S node
                goals.append(PgNode_s(goal, True))

        for goal in goals:
            for count, level in enumerate(self.s_levels):
                if goal in level:
                    level_sum += count
                    break
            else:
                """for l in range(len(self.a_levels)):
                    print('A level ', l)
                    for a_node in self.a_levels[l]:
                        a_node.show()

                    print('S level ', l)
                    for s_node in self.s_levels[l]:
                        s_node.show()

                print('S level ', l + 1)
                for s_node in self.s_levels[l + 1]:
                    s_node.show()"""
